...
CREATE TABLE new_item_growth_plan_tstage_June_2021_cohort_item_base_inform
ation_1 AS
SELECT item_id
 ,ds
 ,reserve_price
 ...
 FROM_UNIXTIME(
 CAST(
 keyvalue(features, ';', ':', 'first_starts_time') AS B
IGINT
 )/1000
 )
 ,'yyyymmdd'
...
AND ds IN ('20210630')
AND SUBSTR(to_char(FROM_UNIXTIME(CAST(keyvalue(features, ';', ':', 'fi
rst_starts_time') AS BIGINT )/1000 ),'yyyymmdd'),1,6) IN ('202106')
AND bc_type = 'B'
AND item_id IN (SELECT item_id FROM lha_research_dev.new_item_growth_p
lan_tstage_june_2021_cohort_item_list);
;